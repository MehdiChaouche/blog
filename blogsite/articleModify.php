...
if (!empty($_POST)){
    ...
    if (!empty($titre) && !empty($contenu)) {
        $createArticle = articleModify($bdd, $titre, $contenu, $articleID);
        echo "<p style='color:#FF0000'>L'article a été modifié avec succès !</p>";
    }
}
...
<header>
    <h1>Formulaire de modification d'article(s)</h1><br>
</header>
<body>
<form method="post">
    <div class="form-group">
        <label for="exampleFormControlInput1">Titre de l'article</label>
        <input type="text" class="form-control" name="titre" value="<?php echo $articleView['titre'] ?>">
    </div>
    <div class="form-group">
        <label for="exampleFormControlTextarea1">Contenu de l'article</label>
        <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="contenu"><?php echo $articleView['texte'] ?></textarea>
    </div>
    ...
    <?php
    $degre = $articleView['degreimportance'];
    $selected_attribute = '';
    if ($_POST['degre']=$degre){
    ...
    <p>Degré d'importance de l'article :
        <select class="custom-select custom-select-sm" name="degre">
            ...
            <option value="4">Modéré</option>
            <option value="5">Anecdotique</option>
        </select>
    </p>
    <br>
    <input class="btn btn-primary btn-lg active" type="submit" value="Modifier" name="SubmitEdit">
</form>
<!-- Optional JavaScript -->
